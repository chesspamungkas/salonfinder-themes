...
$displayDate = new \DateTime($voucherMeta['redeemedDate']);
$expiredDate = new \DateTime($voucherMeta['expiryDate']);
...
<?php if( !$isMobile && !$isIOS ) :?>
  <div class="card-profile">
    ...
    <?php 
    /*if($redeemedVoucher['displayDate']!=='') {
      $status = 'redeemed';
    }
    ?>
    <?php if ($status=='gifted') {?>
      <div class="profile-product voucher-status poppins-medium pt-2 pb-2 pl-5">
    ...
    <?php } elseif ($status=='redeemed') {*/ ?>
      <div class="profile-product voucher-expired poppins-medium pt-3 pb-3 pl-4">
        <i class="fas fa-check mr-2"></i> this voucher has been redeemed on <?= $displayDate->format('d M Y') ?>
      </div>
    <?php //} ?>
    <div class="p-0">
        <table width="100%">
...
<?php else: ?>
  <div class="card-profile">
    ...
    <?php /*if($redeemedVoucher['displayDate']!=='') {
      $status = 'redeemed';
    }
    ?>
    <?php if ($status=='gifted') {?>
      <div class="profile-product voucher-status poppins-medium pt-2 pb-2 pl-3">
        <i class="fas fa-gift mr-2"></i> gifted to recipient!
      </div>
    <?php } elseif ($status=='appointment') {?>
      <div class="profile-product voucher-status poppins-medium pt-2 pb-2 pl-3">
      ...
      <?php } elseif ($status=='expired') {?>
      <div class="profile-product voucher-expired poppins-medium pt-3 pb-3 pl-3">
        <i class="fas fa-check mr-2"></i> this voucher has ended
      </div>
    <?php } elseif ($status=='redeemed') { */ ?>
      <div class="profile-product voucher-expired poppins-medium pt-3 pb-3 pl-3">
